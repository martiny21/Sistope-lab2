...
//Funcion escritura proceso hijo
void SendImagesFather(BMPImage *Images, int N)
{
    for(int i = 0; i<N;i++){
        BMPImage img = Images[i];
        int width = img.width;
        ...
        while(PixelsSent < TotalPixels){
            size_t numPixels = 100;
            if(TotalPixels - PixelsSent < 100 && TotalPixels - PixelsSent > 0){
                numPixels = TotalPixels - PixelsSent;
            }
            write(STDOUT_FILENO, &numPixels, sizeof(size_t));
            RGBPixel *subData = (RGBPixel*)malloc(sizeof(RGBPixel) * numPixels);
            if (subData == NULL){
                fprintf(stderr, "Error al asignar memoria\n");
                exit(EXIT_FAILURE);
            }
            getPixels2(img.data, (int)numPixels, PixelsSent, subData);
            write(STDOUT_FILENO, subData, numPixels * sizeof(RGBPixel));
            free(subData);
            PixelsSent += numPixels;
            if(PixelsSent == TotalPixels){     
                size_t zero = 0;
...
void getPixels2(RGBPixel *data, int Npixels, int visited, RGBPixel *pixels) {
    for (int j = 0; j < Npixels; j++) {
        pixels[j] = data[visited + j];
    }
}
