...
void ReadImages(BMPImage *NewImages, int *fd, int N)
{
    ...
    BMPImage ImageRead;
    int i = 0;
    char resultado[500] = "resultado";
    char bmp[] = ".bmp";
    while (i < N)
    {
        read(fd[READ], &ImageRead, sizeof(BMPImage));
        strcat(resultado, bmp);
        write_bmp(resultado, &ImageRead);
        i += 1;
    }
}
...
void ReadImages2(BMPImage *NewImages, int *fd, int *fd2, int N)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        read(fd[READ], &width, sizeof(int));
        read(fd[READ], &height, sizeof(int));

        BMPImage img;
        img.width = width;
        img.height = height;
        RGBPixel *data = malloc(width * height * sizeof(RGBPixel));
        ...
        while (1)
        {
            ...
            if (numPixels == 0)
            {
                fprintf(stderr, "Broker: imagen leida\n");
                break;
            }
            RGBPixel *Buffer = (RGBPixel *)malloc(sizeof(RGBPixel) * numPixels);
            read(STDIN_FILENO, Buffer, sizeof(RGBPixel) * numPixels);

            memcpy(data + PixelsReady, Buffer, numPixels * sizeof(RGBPixel));
            PixelsReady += (int)numPixels;

            write(fd2[WRITE], &done, sizeof(int));
            free(Buffer);
        }
        img.data = data;
        NewImages[i] = img;
