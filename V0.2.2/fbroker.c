...
void brokerProcess(int *BrokerToWorker, int *WorkerToBroker, int numberPixels, RGBPixel *data, RGBPixel *NewData, char *argv[]){
    ...
    size_t numberSent = 1000;
    if(numberPixels < 1000){
    ...
    while(1){
        if (PixelsSent + numberSent < numberPixels){
            ...
            if(read(WorkerToBroker[READ], NewData, numberSent * sizeof(RGBPixel)) != numberSent * sizeof(RGBPixel)){
                fprintf(stderr,"Broker: Error al leer del pipe\n");
                exit(EXIT_FAILURE);
            }

            putPixels(subData, NewData, numberSent, PixelsSent);
            /*
            ...
            */
            ...
            if(lastPixels <= 0){
                write(BrokerToWorker[WRITE], 0, sizeof(size_t));
                break;
            }
        ...
        if(PixelsSent < numberPixels){
            if(numberPixels - PixelsSent > 1000){
